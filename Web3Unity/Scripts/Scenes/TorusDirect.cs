...
public class TorusDirect : TorusDirectMonoBehaviour
{
    ...
    void OnPostLogin(TorusEvent ev)
    {
        ...
        // save to _config.account
        _Config.Account = ev.credentials.publicAddress;
        PlayerPrefs.SetString("privateKey", ev.credentials.privateKey);

        // move to next scene
        SceneManager.LoadScene(SceneManager.GetActiveScene().buildIndex + 1);

        Debug.Log($"Login succeeded: {ev.credentials.publicAddress + "-" + ev.credentials.privateKey}");
    }
}
